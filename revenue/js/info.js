$(document).ready(function(){
	...
	$('#addBeneficiery').click(function(){
		...
		$('#studentForm')[0].reset();		
		$('.modal-title').html("<i class='fa fa-plus'></i> Beneficuary Info");
		$('#action').val('addBeneficiery');
		$('#save').val('Save');
	});	
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data){
				$('#studentModal').modal('show');
				$('#sr').val(data.sr);
				$('#sr_year').val(data.sryear);
				$('#file_name').val(data.fileno);
				$('#file_year').val(data.fileyear);
				if(data.gender == 'male') {
	...
	$(document).on('click', '.delete', function(){
		...
		if(confirm("Are you sure you want to delete this beneficuary info?")) {
			$.ajax({
				url:"action.php",
				method:"POST",
				data:{studentid:studentid, action:action},
